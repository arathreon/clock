...
fn decrease_hours_decreases_hours() {
    let mut time = Time {hours: 12, minutes: 0};
...
fn increase_hours_moves_to_0_from_23() {
    let mut time = Time {hours: 23, minutes: 0};
    increase_hours(&mut time);
...
fn increase_minutes_increases_minutes() {
    let mut time = Time {hours: 12, minutes: 30};
...
fn increase_minutes_increases_minutes_and_hours() {
    let mut time = Time {hours: 12, minutes: 59};
    increase_minutes(&mut time);
    assert_eq!(time.hours, 13);
...
fn decrease_minutes_decreases_minutes_and_hours() {
    let mut time = Time {hours: 12, minutes: 00};
    decrease_minutes(&mut time);
    assert_eq!(time.hours, 11);
...
fn ui_builder() -> impl Widget<Time> {
    ...
    let increment_hours = Button::new("+")
        .on_click(|_ctx, data: &mut Time, _env| increase_hours(data));
    let decrement_hours = Button::new("-")
        .on_click(|_ctx, data: &mut Time, _env| decrease_hours(data));
    let increment_minutes = Button::new("+")
        .on_click(|_ctx, data: &mut Time, _env| increase_minutes(data));
    let decrement_minutes = Button::new("-")
        .on_click(|_ctx, data: &mut Time, _env| decrease_minutes(data));
    ...
    let clock = Painter::new(|ctx: &mut PaintCtx, data: &Time, _: &Env| {
        ...
        for n in 0..12 {
            ...
            ctx.stroke(Line::new((x*(WINDOW_SIZE / 4. - WINDOW_SIZE / 40. * 2.)+(WINDOW_SIZE / 4.), y*(WINDOW_SIZE / 4. - WINDOW_SIZE / 40. * 2.)+(WINDOW_SIZE / 4.)), (x*(WINDOW_SIZE / 4. - WINDOW_SIZE / 100.)+(WINDOW_SIZE / 4.), y*(WINDOW_SIZE / 4. - WINDOW_SIZE / 100.)+(WINDOW_SIZE / 4.))), &Color::BLACK, WINDOW_SIZE / 100.);
        }
        for n in 0..60 {
            let x = (n as f64 / 60.*2.*PI).cos();
            let y = (n as f64 / 60.*2.* PI).sin();
            ctx.stroke(Line::new((x*(WINDOW_SIZE / 4. - WINDOW_SIZE / 40.)+(WINDOW_SIZE / 4.), y*(WINDOW_SIZE / 4. - WINDOW_SIZE / 40.)+(WINDOW_SIZE / 4.)), (x*(WINDOW_SIZE / 4. - WINDOW_SIZE / 100.)+(WINDOW_SIZE / 4.), y*(WINDOW_SIZE / 4. - WINDOW_SIZE / 100.)+(WINDOW_SIZE / 4.))), &Color::BLACK, WINDOW_SIZE / 200.);
        }
        
        let minutes_x = (data.minutes as f64 / 60. * 2. * PI  - PI/2.).cos();
        let minutes_y = (data.minutes as f64 / 60. * 2. * PI  - PI/2.).sin();

        let hours_x = (((data.hours as f64) % 12. + data.minutes as f64 / 60.) / 12.  * 2. * PI  - PI/2.).cos();
        let hours_y = (((data.hours as f64) % 12. + data.minutes as f64 / 60.) / 12. * 2. * PI  - PI/2.).sin();
        ...
        for n in 0..12 {
            ...
            let text_position = (x*(WINDOW_SIZE/40.*7.25) - text_size.width / 2. + (WINDOW_SIZE / 4.), y*(WINDOW_SIZE/40.*7.25) - text_size.height / 2. + (WINDOW_SIZE / 4.));

            ctx.draw_text(&text_layout, text_position);
        }

        ctx.stroke(Line::new(((WINDOW_SIZE / 4.), (WINDOW_SIZE / 4.)), (minutes_x*(WINDOW_SIZE/40.*6.5)+(WINDOW_SIZE / 4.), minutes_y*(WINDOW_SIZE/40.*6.5)+(WINDOW_SIZE / 4.))), &Color::BLACK, WINDOW_SIZE / 100.);
        ctx.stroke(Line::new(((WINDOW_SIZE / 4.), (WINDOW_SIZE / 4.)), (hours_x*(WINDOW_SIZE/40.*3.25)+(WINDOW_SIZE / 4.), hours_y*(WINDOW_SIZE/40.*3.25)+(WINDOW_SIZE / 4.))), &Color::BLACK, WINDOW_SIZE / 100.);
    });
    
    Flex::column()
        .with_child(SizedBox::new(clock).width(WINDOW_SIZE / 2.).height(WINDOW_SIZE / 2.))
        .with_spacer(4.0)
        .with_child(
            Flex::row()
                .with_child(
...
fn main() {
    ...
    AppLauncher::with_window(main_window)
        .log_to_console()
        .launch(Time {hours: 12, minutes: 0})
        .unwrap()
}
